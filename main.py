...
def get_args_parser():
    ...
    parser.add_argument('--mvrw_weights', type=list, default=[True, False], 
                        help='whether to use the multi-view random walk strategy and whether to use the view weight of the samples')
    parser.add_argument('--n_heads', type=int, default=0, help='number of multi-view attention head')
    parser.add_argument('--n_neibs', type=int, default=5, help='number of neighbors in each view')
    parser.add_argument('--n_rw', type=int, default=10, help='round number of random walking')
    parser.add_argument('--n_steps', type=int, default=5, help='number of hops in one random walking')
    parser.add_argument('--knn4mvrw', type=bool, default=False, help='whether to use the knn to build graph for multi-view random walking')
    parser.add_argument('--ratio_nodes', type=float, default=1.0, help='the ratio of nodes used for multi-view random walking')
...
def train_one_time(args, state_logger):
    ...
    model = RWDCL(layer_dims=args.encoder_dim,
                   temperature=args.temperature,
                   n_classes=args.n_classes, 
                   use_layer_norm=args.use_layer_norm,
                   mvrw_weights=args.mvrw_weights,
                   n_heads=args.n_heads,
                   n_neibs=args.n_neibs,
                   n_rw=args.n_rw, 
                   n_steps=args.n_steps,
                   knn4mvrw=args.knn4mvrw,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.print_this_epoch:
            state_logger.write('Epoch {} K-means: NMI = {:.4f} ARI = {:.4f} F = {:.4f} ACC = {:.4f} PUR = {:.4f}'
                               .format(epoch, train_state['nmi'], train_state['ari'], train_state['f'],
                                       train_state['acc'], train_state['purity']))
    return train_state
...
def main(args):
    ...
    result_avr = {'nmi': [], 'ari': [], 'f': [], 'acc': [], 'purity': []}
    ...
    state_logger = utils.FileLogger(os.path.join(args.output_dir, 'log_train_knn4g.txt'))
    ...
    state_logger.write('Average K-means Result: ACC = {:.2f}({:.2f}) NMI = {:.2f}({:.2f}) ARI = {:.2f}({:.2f}) PUR = {:.2f}({:.2f})'
                       .format(*result_avr['acc'], *result_avr['nmi'], *result_avr['ari'], *result_avr['purity']))
...
if __name__ == '__main__':
    ...
    folder_name = '_'.join(
        [args.dataset,
         'tau', str(args.temperature), 'bs', str(args.batch_size), 'lr', str(args.lr)])
